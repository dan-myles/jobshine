...
/*
 * Amount of steps in the carousel
 */
const TOTAL_STEPS = 4

/**
 * Steps for the carousel
 * These are just default steps, but props are prioritized over these.
 */
const STEPS = [
  {
    id: "1",
...
function FeatureCard({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          `group relative w-full overflow-hidden rounded-3xl border border-black/10
          bg-gradient-to-b from-neutral-900/90 to-stone-800 transition duration-300
          dark:from-neutral-950/90 dark:to-neutral-800/90`,
...
function Steps({
  ...
  return (
    <nav aria-label="Progress" className="flex justify-center px-4">
      <ol
        className="flex w-full flex-wrap items-start justify-start gap-2 sm:justify-center
          md:w-10/12 md:divide-y-0"
        role="list"
      >
        {steps.map((step, stepIdx) => {
          ...
          return (
            <motion.li
              ...
              className={cn(
                `relative z-10 rounded-full px-3 py-1 transition-all duration-300 ease-in-out
                md:flex`,
            ...
            >
              <div
                className={cn(
                  `group flex w-full cursor-pointer items-center focus:outline-none
      ...
      switch (step) {
        ...
        case 3:
          ...
          return (
            <motion.div
              className={clsx(
                `absolute top-1/3 left-2/4 flex w-[100%] -translate-x-1/2 -translate-y-[33%]
                flex-col gap-12 text-center text-2xl font-bold md:w-[60%]`,
              )}
            ...
            >
              <AnimatedStepImage
                alt={image.alt}
                className="pointer-events-none top-[50%] w-[90%] overflow-hidden rounded-2xl border
                  border-neutral-100/10 md:top-[30%] md:left-[35px] md:w-full dark:border-zinc-700"
                preset="fadeInScale"
                // TODO: Replace with some cult image
  ...
  return (
    <FeatureCard {...props} step={step}>
      ...
      <motion.div
        ...
        className="absolute top-5 left-[12rem] z-10 h-full w-full cursor-pointer md:left-0"
      >
        <Steps current={step} onChange={() => {}} steps={STEPS} />
      </motion.div>
      <motion.div
        className="absolute top-0 right-0 z-10 h-full w-full cursor-pointer md:left-0"

...
export async function resumeAI(resume: Resume, jobDescription: string) {
  const sysPrompt = syspromptt()
...
You will not include any backticks in your response, your response should
NOT be formatted in a markdown code block. You are an API, and will act as an
API that returns JSON that can be properly formatted and parsed.
You will not include any other text in your response. You will not include
any markdown in your response. After the # Input line, you will see the input,
thay you are going to use to generate the resume.
